...
class IndicadoresController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('cruds.reportes.indicadores.indicadoresIndex');
        /*
        $rutas = [];
        $acciones = [];
        $nombres = [];
        ...
        return view('cruds.reportes.indicadores.consultarIndicadores', compact('data', 'acciones', 'rutasNombre', 'EmpresasNombre', 'dataEmpresas'));
        */
    }
    ...
    public function detalle($shop, $anio, $mes){
        ...
        //select Id,ClientSerialNumber,SUM(ClientCost) AS TotalSum
        ...
        foreach ($formulas as $keyFor => $valueFor) {
            ...
            foreach ($arrayCuentas as $key => $value) {
                ...
                //$formulaValor .= str_replace($value, $arrayCuentavalor[$value], $valueFor->formula);

                //$valueFor->formulaValores =  str_replace($value, $arrayCuentavalor[$value], $valueFor->formula);
            }

            //$valorTotal;
            /*if (array_key_exists($valueFor->codigosri, $formulario)){
                $valueFor->valorTransaccion = $formulario[$valueFor->codigosri]["valorTransaccion"];
            }else{
                $valueFor->valorTransaccion = 0;
            }

            $total += $valueFor->valorTransaccion;*/
        ...
        foreach ($formulas as $keyFor => $valueFor) {
            ...
            foreach ($formulaDividida as $key => $value) {
                ...
                if($this->validaKey($arrayCuentavalor, $value)) {
                    //$formulaDividida[$key] = $arrayCuentavalor[$valueFor->formula][$value];
            ...
            //$bandera = @eval($formulaValores . "; return true;");
        ...
        return view('cruds.reportes.indicadores.detalleIndicadores', compact('formulas', 'anio', 'nombreMes', 'mes','userEmpresa', 'shop'));
    }
    ...
    public function indicadores($shop){
        $formulas = Formulas::where('shop_id', $shop)->get();

        return view('cruds.reportes.indicadores.principal', compact('formulas', 'shop'));
    }

    public function generarPDF($shop, $formula, $valores){

        $jsonVariables = json_decode($valores);

        $shopData = Shop::find($shop);
        $userEmpresa = UserEmpresa::find($shopData->user_empresas_id);
        $user = User::find(Auth::user()->id);

        $formula = Formulas::find($formula);
        ...
        $pdf->loadView('cruds.reportes.pdfIndicadores', [
        ...
        return $pdf->download('IndicadoresFinancieros_'.$userEmpresa->razon_social.'.pdf');
    }
}
